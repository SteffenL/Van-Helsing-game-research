...
LRESULT StorageItemsTabPage::onInitDialog(HWND, LPARAM)
{
    ...
    m_itemList.AddColumn(_T("Condition"), 6);
...
void StorageItemsTabPage::OnOpenGameSave(vanhelsing::engine::StorageGameSave* gameSave)
{
    ...
    int i = 0;
    for (auto item : items) {
        auto& name = nowide::widen(item->GetName());
        CString slotNumber;
        ...
        CString rarity = GameData::Get().GetRarityText(item->Rarity);
        CString quality = GameData::Get().GetQualityText(item->Quality);
        CString condition;
        //condition.Format(_T("%d"), item->);
        ...
        m_itemList.SetItemText(i, 4, rarity);
        m_itemList.SetItemText(i, 5, quality);
        m_itemList.SetItemText(i, 6, condition);
        m_itemList.SetItemText(i, 7, identified);
        ++i;
    }
}
